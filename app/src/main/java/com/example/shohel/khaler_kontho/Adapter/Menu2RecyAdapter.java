...
public class Menu2RecyAdapter extends RecyclerView.Adapter<Menu2RecyAdapter.MyViewHolder> {
   ...
   /* public MenuRecyAdapter(Context context, List<CommonNewsItem> hlist) {
    ...
    public Menu2RecyAdapter(Context context, List<String> heading3) {
        this.context = context;
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        if (!TextUtils.isEmpty(heading3.get(position))){
            holder.menu.setText(heading3.get(position));
        }else{
            holder.menu.setText("");
        }
